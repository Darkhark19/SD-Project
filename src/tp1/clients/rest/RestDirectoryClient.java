...
public class RestDirectoryClient extends RestClient implements Directory {
    ...
    private final static String ACC_USER_ID = "accUserId";
    ...
    @Override
    public Result<Void> deleteClientFiles(String user) {
        return super.reTry( () -> clt_deleteClientFiles(user));
    }

    private Result<Void> clt_deleteClientFiles(String u) {
        Response r = target.path(u)
    ...
    private Result<Void> clt_shareFile(String filename, String userId, String userIdShare, String password) {
        Response r = target.path(userId).path(filename).path(SHARE).path(userIdShare)
                ...
                .request()
                .accept(MediaType.APPLICATION_JSON)
                .post(Entity.entity(userIdShare, MediaType.APPLICATION_JSON));
